...
class Render():
    def __init__(self, device='cuda:0') -> None:
        # model
        self.device = device
        spectre_cfg.pretrained_modelpath = "external/spectre/pretrained/HDTF_pretrained/00032000.tar"
    ...
    def infer(self, exp, exp_gt=None, render_batch=100):
        'input: expression coefficients'
        'output: mesh'
        n = exp.shape[1]
        coefficients = {}
        coefficients['pose'] = torch.zeros((n, 6), dtype=torch.float).to(exp.device)
        coefficients['exp'] = torch.zeros((n, 50), dtype=torch.float).to(exp.device)
        coefficients['shape'] = torch.zeros((n, 100), dtype=torch.float).to(exp.device)
        coefficients['tex'] = torch.zeros((n, 50), dtype=torch.float).to(exp.device)
        coefficients['cam'] = torch.zeros((n, 3), dtype=torch.float).to(exp.device)
        coefficients_pred = copy.deepcopy(coefficients)
        # cam = torch.tensor([8.8093824, 0.00314824, 0.043486204]).unsqueeze(0).repeat(n, 1).to(exp.device)  # cam
        cam = torch.tensor([8.740263, -0.00034628902, 0.020510273]).unsqueeze(0).repeat(n, 1).to(exp.device)  # cam
        for key in coefficients:
            # coefficients[key] = torch.FloatTensor(torch.from_numpy(coefficients[key])).to(exp.device)
            # coefficients_pred[key] = torch.FloatTensor(torch.from_numpy(coefficients_pred[key])).to(exp.device)
            if key == 'exp':
    ...
    def exp2mesh(self, coefficients_info, pose0=True, render_batch=100):
        ...
        if coefficients_info.shape[-1] == 53:
            coefficients = {}
            coefficients['pose'] = torch.zeros((n, 6), dtype=torch.float).to(coefficients_info.device)
            coefficients['exp'] = torch.zeros((n, 50), dtype=torch.float).to(coefficients_info.device)
            coefficients['shape'] = torch.zeros((n, 100), dtype=torch.float).to(coefficients_info.device)
            coefficients['tex'] = torch.zeros((n, 50), dtype=torch.float).to(coefficients_info.device)
            coefficients['cam'] = torch.zeros((n, 3), dtype=torch.float).to(coefficients_info.device)
            cam = torch.tensor([8.740263, -0.00034628902, 0.020510273]).unsqueeze(0).repeat(n, 1).to(coefficients_info.device)  # cam
            for key in coefficients:
                # coefficients[key] = torch.FloatTensor(torch.from_numpy(coefficients[key])).to(coefficients_info.device)
                # coefficients_pred[key] = torch.FloatTensor(torch.from_numpy(coefficients_pred[key])).to(coefficients_info.device)
                if key == 'exp':
            ...
            cam = torch.tensor([8.740263, -0.00034628902, 0.020510273]).unsqueeze(0).repeat(n, 1).to(coefficients_info.device)  # cam
            for key in coefficients:
                coefficients[key] = torch.FloatTensor(coefficients[key]).to(coefficients_info.device)
                if pose0:
                    if key == 'pose':
    ...
    def coff2mesh(self, coeff, pose0=True, render_batch=100):
        n = coeff.shape[0]

        assert coeff.shape[-1] == 209 or coeff.shape[-1] == 213 or coeff.shape[-1] == 236
        coefficients = get_coefficients(coeff)
        cam = torch.tensor([8.740263, -0.00034628902, 0.020510273]).unsqueeze(0).repeat(n, 1).to(coeff.device)  # cam
        coefficients['cam'] = cam
        if pose0:
            coefficients['pose'][:, :3] = torch.zeros_like(coefficients['pose'][:, :3])
    ...
    def coff2mesh_rawcam(self, coeff, pose0=True, render_batch=100):
        n = coeff.shape[0]

        if coeff.shape[-1] == 209 or coeff.shape[-1] == 213 or coeff.shape[-1] == 236:
            coefficients = get_coefficients(coeff)
            if pose0:
                for key in coefficients:
                    if key == 'pose':
